    ...
    var waterFull = (function () {
        ...
        function bindHTML() {
            ...
            function queryHTML(obj) {
                var {id,pic,number,price,link} = obj;
                return `
                        <div>
                        <div class="pict"><img src="${pic}" alt=""></div>
                        <p>${number}</p>
                        <p><span>￥</span><span>${price}</span></p>
                        <a class="btn" target="_blank" href="${link}">加入购物车</a>
                        </div>`;
            }

            //拿到数据
            function callBackDta(jsonData) {
                for (var i = 0; i < jsonData.length; i += 4) {
                    //i 0 3 6 9 ....
                    //BUG：我们在循环进来的时候i+=3,如果正好是3的倍数，正好可以获取玩，如果不是会存在有一项或两项不存在
                    var item1 = jsonData[i];
                    var item2 = jsonData[i + 1];
                    var item3 = jsonData[i + 2];
                    var item4 = jsonData[i + 3];
                    ...
                    if (item4) {
                        boxList[3].innerHTML += queryHTML(item4);
                    }
